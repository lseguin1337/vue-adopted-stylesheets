...
function installSheet(app, sheet) {
  if (app._container) {
    ...
    // countainer not present yet waiting for the app to be mounted
...
function unistallSheet(app, sheet) {
  ...
  if (!rootNode?.adoptedStyleSheets?.length) return;
  rootNode.adoptedStyleSheets = rootNode.adoptedStyleSheets.filter((s) => s !== sheet);
}

function createStyleSheet(styles) {
  const sheet = new CSSStyleSheet();
  // sheet.replace is not available into testing environment
  sheet.replace?.(styles.join('\n'));
